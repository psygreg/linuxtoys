...
### Cenários de Teste:
* **Remoção com Sucesso:** Executar o script de reversão -> A pasta dedicada da fonte é deletada, o cache é limpo e a fonte deixa de constar no sistema.
* **Diretório Inexistente:** Executar a reversão em um sistema que nunca instalou a fonte -> O script valida que o diretório não existe e encerra informando que o sistema já está no estado padrão, sem disparar erros de comando.
...
### Cenários de Teste:
* **Mudança de Perfil Ativa:** Executar o script com Pipewire ativo -> Canais são mapeados para Mono com sucesso.
* **Servidor de Áudio Ausente:** Executar o script em um servidor sem subsistema de som configurado -> O script falha graciosamente notificando que o gerenciador de áudio não foi encontrado.
...
Então o tamanho da fonte do terminal de monitoramento é ajustado proporcionalmente.

### Cenários de Teste:
* **Argumento Válido:** Passar o parâmetro de aumento (ex: +2) -> Fonte do terminal aumenta de tamanho.
* **Argumento Inválido/Campo Vazio:** Passar um caractere de texto ou nenhum valor -> O script aplica uma validação de tipo, rejeita a entrada e mantém a configuração padrão de segurança.
...
### Cenários de Teste:
* **Injeção de Estilo:** Aplicar o patch CSS na interface web -> Elementos visuais aumentam a área útil de clique.
* **Incompatibilidade de Elemento:** Arquivo de layout original modificado ou ausente -> O validador interrompe a aplicação do patch para não quebrar a estrutura visual da página.