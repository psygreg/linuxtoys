...
case $PACKAGE_MANAGER in
    dnf)
        ...
        PACKAGES="python3 python3-devel zenity jq gobject-introspection-devel cairo-devel cairo cairo-gobject-devel gtk3 curl wget git vte291 gcc pkg-config"
        ;;
    apt)
        ...
        PACKAGES="python3 python3-dev zenity jq libgirepository1.0-dev libcairo2-dev libcairo2 libcairo-gobject2 libgtk-3-dev curl wget git libvte-2.91-dev gcc pkg-config"
        ;;
    pacman)
        INSTALLER="sudo pacman -S --noconfirm"
        # Arch names
        PACKAGES="python zenity jq gobject-introspection cairo gtk3 curl wget git vte3 gcc pkgconf"
        ;;
    zypper)
