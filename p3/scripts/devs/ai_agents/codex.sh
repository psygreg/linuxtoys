...
# Ensure .local/bin is in the current session PATH regardless of rc files,
# otherwise command -v may fail right after install when the rc already has the line
# (e.g. reinstall/update) while the current shell session has not picked it up yet.
