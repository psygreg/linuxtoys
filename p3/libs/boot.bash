...
bootloader_upd() {
    if ! is_ostree; then
        ...
        if is_fedora || is_suse || is_rhel; then
            ...
            { sudo_ clr-boot-manager update || sudo_ grub-mkconfig -o /boot/grub/grub.cfg; } || fatal "Unable to update bootloader"
            _append_transmap "updated bootloader"
        fi
    else
        return 0
...
initramfs_upd() {
    if is_debian || is_ubuntu; then
        sudo_ update-initramfs -u -k all || die "Failed to update initramfs"
    elif is_arch || is_cachy; then
        if command -v dracut &> /dev/null; then
            sudo_ dracut -f --regenerate-all || die "Failed to update dracut"
        else
            sudo_ mkinitcpio -P || die "Failed to update mkinitcpio"
        fi
...
kargs_upd() {
    for arg in "$@"; do
        sudo_ rpm-ostree kargs --append="$arg" || fatal "Failed to update rpm-ostree kargs"
...
grubbyargs_upd () {
    ...
    for arg in "$@"; do
        sudo_ grubby --args="$arg" --update-kernel ALL || fatal "Failed to update grubby kargs"
...
secureboot_check() {
    ...
    sudo_ mokutil --sb-state | grep -q "SecureBoot enabled" || return 0
    if is_fedora || is_rhel || is_ostree; then
        call_script modsign
    elif is_ubuntu; then
        { sudo_ mokutil --test-key /var/lib/shim-signed/mok/MOK.der | grep -q "not enrolled" && \
            { sudo_ update-secureboot-policy --enroll-key || die "Failed to update secure boot policy"; }; } || true
    elif is_debian; then
        if [ "$_ubuntumok" -eq 1 ] && [ ! -f /var/lib/shim-signed/mok/MOK.der ]; then
            ...
            sudo_ mokutil --import /var/lib/shim-signed/mok/MOK.der || die "Failed to create ubuntu-like key"
        else
            { sudo_ mokutil --test-key /var/lib/dkms/mok.pub | grep -q "not enrolled" && \
                { sudo_ mokutil --import /var/lib/dkms/mok.pub || die "Failed to update secure boot policy"; }; } || true
        fi
    fi
}