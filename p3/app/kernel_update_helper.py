...
def get_current_kernel():
    ...
    try:
        ...
        if "psycachy" in kernel_string.lower():
...
def check_for_kernel_updates(verbose=False):
    ...
    if releases is None:
        if verbose:
            print(
                "Could not check for kernel updates (network error or API unavailable)"
            )
...
def show_kernel_update_dialog(
    kernel_type, current_version, latest_version, translations=None
):
    """
    ...
    if os.environ.get("LT_MANIFEST") != "1":
        try:
            return _show_gtk_kernel_update_dialog(
                kernel_type, current_version, latest_version, translations
            )
        except ImportError:
...
def _show_gtk_kernel_update_dialog(
    kernel_type, current_version, latest_version, translations=None
):
    """
    ...
    try:
        ...
        dialog = Gtk.Dialog(
            title=translations.get(
                "kernel_update_available_title", "Kernel Update Available"
            )
            if translations
            else "Kernel Update Available",
        ...
        install_btn = dialog.add_button(
            translations.get("kernel_update_install_btn", "Install Update")
            if translations
            else "Install Update",
            Gtk.ResponseType.YES,
        )
        dialog.add_button(
            translations.get("kernel_update_ignore_btn", "Ignore")
            if translations
            else "Ignore",
            Gtk.ResponseType.NO,
        )
        ...
        message_text = (
            translations.get(
                "kernel_update_available_message",
                f"A new version of {kernel_display} kernel is available.",
            )
            if translations
            else f"A new version of {kernel_display} kernel is available."
        )
        ...
        info_text = (
            translations.get(
                "kernel_update_reboot_required",
                "A system reboot will be required after installation.",
            )
            if translations
            else "A system reboot will be required after installation."
        )
...
def run_psycachy_installer(kernel_type):
    ...
    try:
        ...
        script_dir = os.path.join(
            os.path.dirname(os.path.dirname(__file__)), "scripts", "extra"
        )
        ...
        subprocess.Popen(["bash", script_path, flag], stdin=subprocess.PIPE, env=env)
...
def run_kernel_update_check(show_dialog=True, verbose=False, translations=None):
    ...
    is_update_available, kernel_type, current_version, latest_version = (
        check_for_kernel_updates(verbose)
    )
...
if __name__ == "__main__":
    ...
    if run_kernel_update_check(
        show_dialog=not no_dialog, verbose=verbose, translations=None
    ):
